...
const associateSchema = new mongoose.Schema({
    ...
    is_form_submitted: { type: Boolean, default: false },
    is_welcome_email_send: { type: Boolean, default: false },
    is_sms_send: { type: Boolean, default: false },
    term_condition: { type: Boolean, default: false },
    active: { type: Boolean, default: true },
    ..._commonKeys
}, { timestamps: true });

// Pre-save hook to generate a unique user code for new associates
associateSchema.pre('save', async function (next) {
    if (!this.isNew) return next();
    const Associate = mongoose.model(_collectionName.Associate, associateSchema);
    try {
        const lastAssociate = await Associate.findOne().sort({ createdAt: -1 });
        let nextUserCode = 'AS00001';
        if (lastAssociate && lastAssociate.user_code) {
            const lastCodeNumber = parseInt(lastAssociate.user_code.slice(2));
