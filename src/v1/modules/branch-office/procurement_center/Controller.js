...
//         };
//         if (associateName) {
//             query["user_id._id"] = convertToObjecId(associateName);
//         }
...
module.exports.getProcurementCenter = async (req, res) => {
    try {
        ...
        if (state) {
            matchQuery["address.state_id"] = convertToObjecId(state);
        }
        ...
        const countResult = await ProcurementCenter.aggregate(countPipeline);
        const totalCount = countResult.length > 0 ? countResult[0].count : 0;

        // Apply pagination if not export
        let rowsPipeline = [...pipeline];
        if (paginate == 1 && isExport != 1) {
            rowsPipeline.push({ $skip: parseInt(skip) });
            rowsPipeline.push({ $limit: parseInt(limit) });
        }

        const rows = await ProcurementCenter.aggregate(rowsPipeline);
        ...
        if (paginate == 1 && isExport != 1) {
            records.page = page;
            records.limit = limit;
            records.pages = limit != 0 ? Math.ceil(totalCount / limit) : 0;
        }

        if (isExport == 1) {
            let record = rows.map((item) => ({
                "Center ID": item?.center_code || 'NA',
                "Center Name": item?.center_name || 'NA',
                "Contact": item?.point_of_contact?.mobile || 'NA',
                "Email": item?.point_of_contact?.email || 'NA',
            ...
            if (record.length > 0) {
                ...
                return res.status(400).send(new serviceResponse({
                    status: 400,
                    data: records,
                    message: _response_message.notFound("collection center")
                }));
            }
        } else {
