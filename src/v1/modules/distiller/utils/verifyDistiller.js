...
exports.verifyDistiller = asyncErrorHandler(async (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET_KEY, async function (err, decodedToken) {
        ...
        if (req.url === '/onboarding' || req.url === '/onboarding-status' || req.url === '/find-user-status' || req.url === '/final-submit'  || req.url === '/final-submit'   || req.url === '/manfacturing-unit'   || req.url === '/storage-facility')  {
            next();
