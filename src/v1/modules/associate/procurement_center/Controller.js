  ...
  try {
    ...
    return res.send(
      new serviceResponse({
        status: 200,
        data: record,
        message: _response_message.created("Collection Center"),
      })
    );
  } catch (error) {
    ...
    records.rows =
      paginate == 1
        ? await ProcurementCenter.find(query)
            ...
            .skip(skip)
            .limit(parseInt(limit))
        : await ProcurementCenter.find(query).sort(sortBy);
    ...
    if (isxlsx) {
      ...
      if (!centers.length) {
        return res.status(400).send(
          new serviceResponse({
            status: 400,
            message: _response_message.notFound("No data found in the file"),
          })
        );
      }
      ...
      return res.status(400).send(
        new serviceResponse({
          ...
          errors: [
            {
              message:
                "Partial upload successfull ! Please export to view the uploaded data.",
            },
          ],
        })
      );
...
module.exports.generateCenterCode = async (req, res) => {
  try {
    ...
    const lastCenter = await ProcurementCenter.findOne({
      center_code: { $exists: true },
    }).sort({ center_code: -1 });
    ...
    while (!isUnique) {
      const existingCenter = await ProcurementCenter.findOne({
    ...
    return res.status(200).send(
      new serviceResponse({
        status: 200,
        data: { CenterCode },
        message: _response_message.found("next center code"),
      })
    );
  } catch (error) {
    _handleCatchErrors(error, res);
