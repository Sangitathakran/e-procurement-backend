...
//                 }
//             },
//             { $unwind: { path: '$myoffer', preserveNullAndEmptyArrays: true } },
...
//                     pipeline: [
//                         { $match: { $expr: { $eq: ["$_id", { $toObjectId: "$$head_office_id" }] } } },
//                         { $project: { headOfficesName: "$company_details.name" } }
//                     ],
//                     as: "headOfficeDetails"
//                 }
//             },
...
//                     pipeline: [
//                         { $match: { $expr: { $eq: ["$_id", "$$sla_id"] } } },
//                         { $project: { slaName: "$basic_details.name" } }
//                     ]
//                     ,
//                     as: "slaDetails"
//                 }
...
//         const pipeline = parseInt(isExport) === 1
//             ? basePipeline
//             : [
//                 ...basePipeline,
//                 ...(parseInt(paginate) === 1 ? [
...
//             const exportData = allRecords.map((item) => ({
//                 "Order Id": item?.reqNo || "NA",
//                 "Commodity": item?.product?.name || "NA",
//                 "Grade": item?.product?.grade || "NA",
//                 "MSP": item?.quotedPrice || "NA",
...
//                 "Expected Delivery Date": item?.deliveryDate || "NA",
//                 "Delivery Location": item?.address?.deliveryLocation || "NA",
//                 "SLA Name": item?.slaName || "N/A",
//                 "Head Office Name": item?.headOfficesName || "N/A",
//                 "CNA": item?.cna || "N/A"
//             }));

//             if (exportData.length > 0) {
//                 await dumpJSONToExcel(req, res, {
//                     data: exportData,
//                     fileName: `Procurement-${Date.now()}.xlsx`,
//                     worksheetName: `Procurement-records`
//                 });
//             } else {
//                 return res.status(400).send(new serviceResponse({
//                     status: 400,
