...
module.exports.createProcurement = async (req, res) => {
    ...
    try {
        ...
        const { organization_id, quotedPrice, deliveryDate, name, grade, variety, quantity, deliveryLocation, lat, long, quoteExpiry, 
            expectedProcurementDate, msp, commodityName, commodityImage, ho, bo, locationUrl } = req.body;
        ...
        const expectedProcurement_date = moment(expectedProcurementDate).format("YYYY-MM-DD");
        ...
        const record = await Request.create({
            ...
            expectedProcurementDate: expectedProcurement_date,
            ho, bo,
            product: { name, grade, variety, quantity, commodityName, commodityImage, msp},
            address: { deliveryLocation, lat, long, locationUrl },
...
module.exports.getProcurement = async (req, res) => {
    ...
    try {
        ...
        const { page, limit, skip, paginate = 1, sortBy, search = '', status, isExport = 0 } = req.query
        ...
        if (isExport == 1) {
            const record = records.rows.map((item) => {
                return {
                    "Request ID": item?.req_id || 'NA',
                    "Commodity ID": item?.product?.commodityName || 'NA',
                    "Grade Name": item?.product?.grade || 'NA'                   
                }
            })
            if (record.length > 0) {
                ...
                dumpJSONToExcel(req, res, {
                    data: record,
                    fileName: `Request-${'Agent'}.xlsx`,
                    worksheetName: `Request-record-${'Agent'}`
                });
            } else {
...
module.exports.getAssociateOffers = asyncErrorHandler(async (req, res) => {
    ...
    if (isExport == 1) {
        const record = records.rows.map((item) => {
            return {
                "Seller ID": item?.sellerOffer_id || 'NA',
                "Request ID": item?.req_id || 'NA',
                "Offer quantity": item?.offeredQty || 'NA',
            }
        })
        if (record.length > 0) {
            ...
            return res.status(200).send(new serviceResponse({ status: 400, data: records, message: _response_message.notFound("seller offer") }))
        }
    } else {
        return res.status(200).send(new serviceResponse({ status: 200, data: records, message: _response_message.found("seller offer") }));
    }
