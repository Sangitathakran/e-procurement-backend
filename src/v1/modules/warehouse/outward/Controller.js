...
module.exports.readyToShip = asyncErrorHandler(async (req, res) => {
    ...
    if (sumOfAllotedQty > purchaseOrderRecord.quantityRequired) {
        return res.status(200).send(new serviceResponse({ status: 400, errors: [{ message: "Alloted quantity exceeds Required quantity!" }] }));
    }
...
module.exports.rejectTrack = asyncErrorHandler(async (req, res) => {
    ...
    const record = new TrackOrder({
        purchaseOrder_id: result._id, 
        status: _trackOrderStatus.rejected,
        rejection: {
            is_reject: true,
            reason: reason
        },
        createdBy: organization_id || null,        
        createdAt: new Date()
    });
    await record.save();

    return res.status(200).send(new serviceResponse({ status: 200, message: _response_message.rejectedSuccessfully("Purchase order") }));
})
