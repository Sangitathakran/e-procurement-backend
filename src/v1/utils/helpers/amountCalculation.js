...
const calculateAmount = async (token, poQuantity, branch_id, session) => {
  try {
    ...
    // Fetch tax document with optional session
    const taxQuery = StateTaxModel.findOne({
      state_name: { $regex: `^${branch.state}$`, $options: 'i' },
    }).lean();
    if (session) taxQuery.session(session);
    const taxDoc = await taxQuery;
    ...
    if (token === 10) {
      advancePercent = _advancePayment();
    } else if (token === 100) {
      mandiTax = taxDoc?.mandi_tax || 0;
      advancePercent = 100;
    }
